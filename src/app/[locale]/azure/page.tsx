...
const conceptNodes: ConceptNode[] = [
  ...
  { id: "key-vault",    label: "Key Vault",       x: 750, y: 0,   group: "security",   shape: "circle" },
  { id: "service-bus",  label: "Service Bus",     x: 100, y: 150, group: "messaging",  shape: "pill" },
  { id: "cosmos",       label: "Cosmos DB",       x: 370, y: 150, group: "data",       shape: "hexagon" },
...
const groupColors: Record<string, string> = { compute: "#0ea5e9", security: "#ef4444", messaging: "#f59e0b", data: "#10b981", networking: "#a855f7", monitoring: "#06b6d4", devops: "#ec4899" };

const sections = [
  { heading: "Compute", badge: "bg-sky-500/15 text-sky-400 border-sky-500/30", items: [
    { title: "Azure Functions", description: "Serverless execution — triggers, bindings, cold starts, and scaling.", href: "/azure/functions" },
    { title: "App Service & Hosting", description: "Plans, auto-scaling, deployment slots, and zero-downtime deployments.", href: "/azure/app-service" },
  ]},
  { heading: "Security", badge: "bg-red-500/15 text-red-400 border-red-500/30", items: [
  ...
  { heading: "Data & Messaging", badge: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30", items: [
  ...
  { heading: "Platform", badge: "bg-purple-500/15 text-purple-400 border-purple-500/30", items: [
    { title: "API Management (APIM)", description: "Gateway pattern, policies, rate limiting, and developer portal.", href: "/azure/api-management" },
    { title: "Application Insights", description: "Telemetry pipeline, sampling, dependency tracking, and live metrics.", href: "/azure/app-insights" },
  ...
  return (
    <MotionFade>
      <div className="max-w-6xl">
        <div className="mb-10">
          <h1 className="text-3xl font-bold mb-3 tracking-tight" style={{ fontFamily: "var(--font-heading)" }}>Azure Cloud Services</h1>
          <p className="text-text-secondary max-w-2xl leading-relaxed">How Azure services work under the hood — from serverless functions and authentication to message queues and observability.</p>
        </div>
        <CategoryConceptTree nodes={conceptNodes} edges={conceptEdges} accentColor="#0ea5e9" groupColors={groupColors} title="Azure Service Connections" height={370} />
