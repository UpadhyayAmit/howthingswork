...
const sections = [
  { heading: "Core", items: [
    { title: "DbContext & Change Tracker", description: "Unit of Work pattern, entity states (Added/Modified/Deleted), and SaveChanges flow.", href: "/entity-framework/dbcontext" },
    { title: "Migrations & Schema Evolution", description: "How migration snapshots track database changes and generate SQL diff scripts.", href: "/entity-framework/migrations" },
  ]},
  { heading: "Query Engine", items: [
    { title: "Query Pipeline (LINQ → SQL)", description: "IQueryable → expression tree → SQL generation — how EF translates your LINQ.", href: "/entity-framework/query-pipeline" },
    { title: "Query Performance & Indexing", description: "Compiled queries, split queries, global query filters, and avoiding N+1.", href: "/entity-framework/query-performance" },
    { title: "Raw SQL & Stored Procedures", description: "FromSqlRaw, ExecuteSqlRaw — when and how to escape the ORM.", href: "/entity-framework/raw-sql" },
  ]},
  { heading: "Schema & Data", items: [
    { title: "Loading Strategies", description: "Eager vs Lazy vs Explicit loading — trade-offs and the N+1 problem.", href: "/entity-framework/loading-strategies" },
    { title: "Relationships & Navigation", description: "FK conventions, shadow properties, cascade delete, and owned types.", href: "/entity-framework/relationships" },
    { title: "Concurrency & Conflict Resolution", description: "Optimistic concurrency with row versioning and conflict detection.", href: "/entity-framework/concurrency" },
  ]},
];
  ...
  return (
    <MotionFade>
      <div className="max-w-6xl">
        <div className="mb-10">
          <h1 className="text-3xl font-bold mb-3 tracking-tight" style={{ fontFamily: "var(--font-heading)" }}>Entity Framework Core</h1>
          <p className="text-text-secondary max-w-2xl leading-relaxed">How EF Core maps your C# objects to database operations — from change tracking and LINQ translation to migrations and performance optimization.</p>
        </div>
        <CategoryConceptTree nodes={conceptNodes} edges={conceptEdges} accentColor="#f59e0b" groupColors={groupColors} title="EF Core Architecture" height={480} />
